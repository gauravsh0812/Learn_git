...
def main():
    ...
    for yr in [14]:#, 15, 16, 17, 18]:
        ...
        json.dump(sorted_error_counter_tuples, open(os.path.join(root, f'{year}/Logs/combine_logs/errors_counter_tuples.json'), 'w'))
...
def Ranking(token_counter, error_counter):
    ...
    top10_tokens_tuples = dict(sorted(sorted_token_counter_tuples.items(), key = itemgetter(1))[:10])
    top10_errors_tuples = dict(sorted(sorted_error_counter_tuples.items(), key = itemgetter(1))[:10])
...
def Distribution(root, year, sorted_token_counter_tuples, sorted_error_counter_tuples, top10_tokens_tuples, top10_errors_tuples):
    ...
    for tuple in [sorted_token_counter_tuples, sorted_error_counter_tuples, top10_tokens_tuples, top10_errors_tuples]:

        # get sorted dictionary form tuple
        temp_dict = dict(tuple)
