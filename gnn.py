...
#cmd = "export TORCH_HOME='/app'"
#os.system(cmd)
#print(TORCH_HOME)
#os.environ['TORCH_HOME']='/app'
#print(os.path.exists('load_models'))
#TORCH_HOME/hub.set_dir('load_models')
#'/app'/hub.set_dir('load_models')
...
if texture_loss:
    ...
    #print('if shapenet dataset:  ', shapenet_dataset)
else:
    shapenet_dataset = ShapeNetCore(shapenet_path, synsets=[category], version=2, load_textures=True,
                                    texture_resolution=4)
    #print('else shapenet dataset:  ', shapenet_dataset.__dict__)
...
if model_type == "resgcnface":
    ...
    netG = NeRF(in_features=in_features, n_hidden=n_hidden, out_features=out_features,
                noise_dim=noise_dim, dropout=dropout).to(device)
elif model_type == "graphunet":
    netG = GraphUNet(in_channels=in_features, hidden_channel=16, out_channels=out_features).to(device)
else:
...
for epoch in range(num_epochs):
    ...
    for idx in tqdm(train_set):
        ...
        raster_settings = RasterizationSettings(bin_size=-1, image_size=image_size, blur_radius=0.0, faces_per_pixel=1,
                                                cull_backfaces=True)
